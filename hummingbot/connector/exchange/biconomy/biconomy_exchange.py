...
class BiconomyExchange(ExchangePyBase):
    ...
    async def get_all_pairs_prices(self) -> List[Dict[str, str]]:
        result = await self._api_get(path_url=CONSTANTS.TICKER_BOOK_PATH_URL)
        pairs_prices = result.get("ticker", []) 
        return pairs_prices
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        cancel_result = await self._api_post(
            ...
            data=api_params,
            is_auth_required=True)
        if cancel_result.get("code") == 0 or cancel_result.get("message") == "Order not found":
            return True
        return False
    ...
    async def _format_trading_rules(self, exchange_info_dict) -> List[TradingRule]:
        ...
        for rule in filter(biconomy_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                ...
                min_price_inc = Decimal(f"1e-{rule['quoteAssetPrecision']}")
                min_amount_inc = Decimal(f"1e-{rule['baseAssetPrecision']}")
                min_notional = Decimal(rule['minQuantity '])
               
                baseAsset = rule["baseAsset"]
                if "-" in baseAsset : baseAsset = baseAsset.replace("-","")

                retval.append(
                    TradingRule(combine_to_hb_trading_pair(base=baseAsset, quote=rule["quoteAsset"]),
                                min_order_size = Decimal(rule.get("tickSize")),
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            for order_id in order_by_exchange_id_map:
                params = {
                    "order_id": order_id,
                    "limit" : 100
                }
            ...
            for trades, trading_pair in zip(results, trading_pairs):
                ...
                for trade in trades["result"]["records"]:
                    ...
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        trade_update = TradeUpdate(
                            ...
                            fill_quote_amount=Decimal(trade["amount"] * trade["price"]),
                        ...
                        self.trigger_event(
                            ...
                            OrderFilledEvent(
                                ...
                                order_id=self._exchange_order_ids.get(str(trade["deal_order_id"]), None),
                                trading_pair=trading_pair,
    ...
    async def _all_trade_updates_for_order(self, order: InFlightOrder) -> List[TradeUpdate]:
        ...
        if order.exchange_order_id is not None:
            ...
            trades = all_fills_response.get("result").get("records")
            quote_asset = trading_pair.split('_')[1]
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        order_status_resp = await self._api_post(
            path_url=CONSTANTS.PENDING_ORDER_DETAILS_PATH_URL,
            data={
                "market": trading_pair,
                "order_id": tracked_order.exchange_order_id},
            is_auth_required=True)
        updated_order_data = order_status_resp["result"]

        if updated_order_data:
            new_state = ( CONSTANTS.ORDER_STATE["PARTIALLY_FILLED"] if float(updated_order_data["deal_stock"]) > 0
            else CONSTANTS.ORDER_STATE["OPEN"] )
            update_timestamp = updated_order_data["mtime"]
        else:
           order_status_resp = await self._api_post(
              path_url=CONSTANTS.FINISHED_ORDER_DETAILS_PATH_URL,
              data={
                "market": trading_pair,
                "order_id": tracked_order.exchange_order_id
              },
           ...
           updated_order_data = order_status_resp["result"]
           new_state = ( CONSTANTS.ORDER_STATE["CANCELED"] if float(updated_order_data["deal_stock"]) < float(updated_order_data["amount"])
           else CONSTANTS.ORDER_STATE["FILLED"] )
           update_timestamp = updated_order_data["ftime"]
    ...
    async def _update_balances(self):
        ...
        for key,value in balances.items():
            asset_name = key
            free_balance = Decimal(value["available"])
            total_balance = Decimal(value["available"]) + Decimal(value["freeze"]) + Decimal(value["other_freeze"])
            self._account_available_balances[key] = free_balance
            self._account_balances[key] = total_balance
            remote_asset_names.add(key)
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        return float(resp_json[0].get("price"))
